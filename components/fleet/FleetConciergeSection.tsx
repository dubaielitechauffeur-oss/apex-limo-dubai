          ...
          {recommendations.map((item, index) => (
            <Reveal key={item.key} delay={index * 80}>
              <Link
                ...
                className="group block rounded-xl border border-gold/15 bg-obsidian-light p-6 transition-colors duration-200 hover:border-gold/50 hover:bg-obsidian-light"
              >
                <h3 className="font-display text-lg text-white transition-colors duration-200 group-hover:text-gold">
                  {item.name}
                </h3>
                <p className="mt-2 text-sm leading-relaxed text-smoke">{item.blurb}</p>
              </Link>
            </Reveal>
          ))}
        </div>
